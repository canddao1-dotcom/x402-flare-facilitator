...
// TipSplitter contract - atomic 99/1 split
const TIP_SPLITTER = {
...
// Protocol fee (handled by TipSplitter contract)
const PROTOCOL_FEE = {
  percent: 1,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (tipMode === 'wallet') {
        ...
        // Total amount (splitter handles the 99/1 split)
        const totalAmount = parseFloat(tipAmount)
        const totalAmountBigInt = parseUnits(totalAmount.toFixed(tokenInfo.decimals), tokenInfo.decimals)
        
        // Get splitter address for this chain
        const splitterAddress = TIP_SPLITTER[selectedChain]
        
        if (!splitterAddress) {
          // Fallback to direct transfer if no splitter on this chain
          setError(`TipSplitter not deployed on ${selectedChain}. Use pool mode.`)
          setLoading(false)
          return
        }
        
        if (tokenInfo.address === 'native') {
          // Native token tip via splitter
          // TODO: Implement native via tipNative
          setError('Native token tips coming soon')
          setLoading(false)
          return
        }
        
        // Step 1: Approve TipSplitter to spend tokens
        await writeContractAsync({
          address: tokenInfo.address,
          abi: ERC20_ABI,
          functionName: 'approve',
          args: [splitterAddress, totalAmountBigInt],
        })
        
        // Step 2: Call TipSplitter.tip() - atomic 99/1 split
        const hash = await writeContractAsync({
          address: splitterAddress,
          abi: TIP_SPLITTER_ABI,
          functionName: 'tip',
          args: [tokenInfo.address, recipientAddress, totalAmountBigInt],
        })
        ...
        // Report tip for statistics tracking
        try {
          await fetch('/api/tip/report', {
            method: 'POST',
          ...
          console.log('Stats report failed (non-critical):', e)
        }
